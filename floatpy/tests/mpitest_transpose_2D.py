...
class TestTranspose2D(unittest.TestCase):
    ...
    def testTransposeInX(self):
        ...
        rho_err = numpy.absolute(rho[lo_p[0]:hi_p[0]+1, lo_p[1]:hi_p[1]+1] - rho_p).max()
        ...
        vel_err = numpy.absolute(vel[lo_p[0]:hi_p[0]+1, lo_p[1]:hi_p[1]+1, :] - vel_p).max()
        ...
        rho_err = numpy.absolute(rho[lo_c[0]:hi_c[0]+1, lo_c[1]:hi_c[1]+1] - rho_c).max()
        self.assertEqual(rho_err, 0.0, "Incorrect transposed data from pencil in x-direction for scalar!")
        
        vel_c = tw.transposeFromPencil(vel_p)
        vel_err = numpy.absolute(vel[lo_c[0]:hi_c[0]+1, lo_c[1]:hi_c[1]+1, :] - vel_c).max()
        self.assertEqual(vel_err, 0.0, "Incorrect transposed data from pencil in x-direction for vector!")
    ...
    def testTransposeInY(self):
        ...
        rho_p = tw.transposeToPencil(rho_c)
        rho_err = numpy.absolute(rho[lo_p[0]:hi_p[0]+1, lo_p[1]:hi_p[1]+1] - rho_p).max()
        self.assertEqual(rho_err, 0.0, "Incorrect transposed data to in y-direction for scalar!")
        
        vel_p = tw.transposeToPencil(vel_c)
        vel_err = numpy.absolute(vel[lo_p[0]:hi_p[0]+1, lo_p[1]:hi_p[1]+1, :] - vel_p).max()
        self.assertEqual(vel_err, 0.0, "Incorrect transposed data to in y-direction for vector!")
        
        rho_c = tw.transposeFromPencil(rho_p)
        rho_err = numpy.absolute(rho[lo_c[0]:hi_c[0]+1, lo_c[1]:hi_c[1]+1] - rho_c).max()
        self.assertEqual(rho_err, 0.0, "Incorrect transposed data from in y-direction for scalar!")
        
        vel_c = tw.transposeFromPencil(vel_p)
        vel_err = numpy.absolute(vel[lo_c[0]:hi_c[0]+1, lo_c[1]:hi_c[1]+1, :] - vel_c).max()
        self.assertEqual(vel_err, 0.0, "Incorrect transposed data from in y-direction for vector!")
