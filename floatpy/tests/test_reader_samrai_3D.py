...
class TestSamraiDataReader3D(unittest.TestCase):
    ...
    def testReadCoordinatesSubdomain(self):
        ...
        self.reader.sub_domain = self.lo, self.hi
        x_s, y_s, z_s = self.reader.readCoordinates()
        
        # Check that the coordinates in sub-domain are equal to the corresponding full coordinates.
        
        x_err = numpy.absolute(x[self.lo[0]:self.hi[0]+1, self.lo[1]:self.hi[1]+1, self.lo[2]:self.hi[2]+1] - x_s).max()
        y_err = numpy.absolute(y[self.lo[0]:self.hi[0]+1, self.lo[1]:self.hi[1]+1, self.lo[2]:self.hi[2]+1] - y_s).max()
        z_err = numpy.absolute(z[self.lo[0]:self.hi[0]+1, self.lo[1]:self.hi[1]+1, self.lo[2]:self.hi[2]+1] - z_s).max()
        ...
        self.assertEqual(z_err, 0.0, "Incorrect sub-domain coordinate data reader in z direction!")
    ...
    def testReadDataSubdomain(self):
        ...
        rho_err = numpy.absolute(rho[self.lo[0]:self.hi[0]+1, self.lo[1]:self.hi[1]+1, self.lo[2]:self.hi[2]+1] - \
                                 rho_s).max()
        vel_err = numpy.absolute(vel[self.lo[0]:self.hi[0]+1, self.lo[1]:self.hi[1]+1, self.lo[2]:self.hi[2]+1, :] - \
                                 vel_s).max()
        p_err   = numpy.absolute(p  [self.lo[0]:self.hi[0]+1, self.lo[1]:self.hi[1]+1, self.lo[2]:self.hi[2]+1] - \
                                 p_s).max()
