...
class wchr_reader:
    ...
    def read_coordinates(self, chunk=None):
        """
        ...
        # Sanitize chunk to be within the domain bounds first
        for i in range(3):
            if chunk[i][0] < 0:
                chunk[i][0] = 0
            if chunk[i][1] > self.domain_size[i]:
                chunk[i][1] = self.domain_size[i]
    ...
    def read_variable(self, var, step, chunk=None):
        """
        Method to read in the a chunk of the data for variable var at vizdump step
        """
    ...
    def plot_three_slice(self, var, index):
        """
    ...
    def cube_show_slider(self, cube, axis=2, **kwargs):
        """
