...
if __name__ == '__main__':
    ...
    for i,qname in enumerate(qlist): 
        ...
        """
        if rank==0: print('upsampling x,z')
        qhat0 = np.fft.fft(np.fft.fft(q0,axis=xdir),axis=zdir) # [Nx0,Ny0,Nz0]
        qhat = np.zeros([Nx,Ny0,Nz],dtype=np.complex64)
        qhat[0:Nx0/2,:,0:Nz0/2] = qhat0[0:Nx0/2,:,0:Nz0/2] # first half
        qhat[(Nx-Nx0)/2:,:,(Nz-Nz0)/2:] = qhat0[-Nx0/2:,:,-Nz0/2:] # second half
        qhat0 = None
