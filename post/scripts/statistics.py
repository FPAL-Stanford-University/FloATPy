...
def integrate_y(f, dy, grid_partition):
    comm = MPI.Comm.f2py(grid_partition.commy())

    myfint = np.array([ np.sum( f ) * dy ])
    fint = np.array([ myfint[0] ])
    comm.Allreduce(myfint, fint, op=MPI.SUM)
    return fint[0]
...
def integrate_xz(f, dx, dz, grid_partition):
    comm = MPI.Comm.f2py(grid_partition.commxz())

    myfint = np.array([ np.sum( f ) * dx * dz ])
    fint = np.array([ myfint[0] ])
    comm.Allreduce(myfint, fint, op=MPI.SUM)
...
def get_enstrophy(vorticity, grid_partition, dx, dy, dz, volume_integrated=False):
    enstrophy = vorticity[:,:,:,0]**2 + vorticity[:,:,:,1]**2 + vorticity[:,:,:,2]**2
    if volume_integrated:
        enstrophy = integrate_volume(enstrophy, dx, dy, dz, grid_partition)
    return enstrophy
...
if __name__ == '__main__':
    ...
    periodic_dimensions = (False,False,True)
    x_bc = (0,1)
    y_bc = (1,1)
    z_bc = (0,0)
    ...
    for step in steps[start_index:]:
        ...
        mixwidth, mixedness = get_mixing_width_mixedness(Y_N2, Y_CO2, reader.grid_partition, avg, dx, dy)
        ...
        if rank == 0:
            line = (' %26.14e'*8) %(reader.time, mixwidth, mixedness, tke, net_circulation, pos_circulation, neg_circulation, enstrophy)
            print line
            fout.write( line + '\n' )
            fout.flush()
