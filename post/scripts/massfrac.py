...
if __name__ == '__main__':
    ...
    for step in tID_list: 
        ...
        ns = 2 #n species
        Y1,Y2 = reader.readData( ('Massfraction_01','Massfraction_02'))
        means = np.zeros([np.shape(Y1)[1],ns],dtype='f')
        means[:,0] = np.squeeze(stats.reynolds_average(avg,Y1))
        means[:,1] = np.squeeze(stats.reynolds_average(avg,Y2))
        ...
        if rank==0:
            total_array = np.zeros([Ny,ns],dtype='f')
            for j in range(ns):
            ...
            outputfile = filename_prefix + 'massfrac_%04d.dat'%step
