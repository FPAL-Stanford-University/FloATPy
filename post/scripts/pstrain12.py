...
if __name__ == '__main__':
    ...
    for tID in tID_list:
        ...
        rbar,utilde,vtilde = None,None,None

        qx,dudy_pp,qz = der.gradient(upp, x_bc=x_bc, y_bc=y_bc, z_bc=z_bc)
        dvdx_pp,qy,qz = der.gradient(vpp, x_bc=x_bc, y_bc=y_bc, z_bc=z_bc)
        upp,vpp = None,None
        qx,qy,qz = None,None,None
      
        tmp = transpose2y(settings,p*(dudy_pp+dvdx_pp))
        Pi12 = stats.reynolds_average(avg,tmp)
        Pi12 = np.squeeze(Pi12)

        if rank==0: 
            outputfile = dir_out+"/shearlayer_pstrain12_%04d.dat"%tID
            np.savetxt(outputfile,Pi12,delimiter=' ')
