...
if __name__ == '__main__':
    ...
    for tID in tID_list:
        ...
        dudx_pp,qy,qz = der.gradient(upp, x_bc=x_bc, y_bc=y_bc, z_bc=z_bc)
        upp = None
        qy,qz = None,None
      
        tmp = transpose2y(settings,pp*(dudx_pp+dudx_pp))
        Pi11 = stats.reynolds_average(avg,tmp)
        Pi11 = np.squeeze(Pi11)

        if rank==0: 
            outputfile = dir_out+"/shearlayer_pstrain11_%04d.dat"%tID
            np.savetxt(outputfile,Pi11,delimiter=' ')
