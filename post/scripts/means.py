...
if __name__ == '__main__':
    ...
    for tid in tID_list:
        ...
        for qname in qlist:
            if qname is 'c':
                q = reader.readData( ('T') )
                c3D = np.sqrt(inp.gam*q[0])
            else:
                q = reader.readData( (qname) )
                q = q[0]
            qbar = stats.reynolds_average(avg,r*q)
            qbar = np.squeeze(qbar)
        
            # now gather from all processes
            root=0
            comm.Barrier()
            recvbuf=comm.gather(qbar, root)
            comm.Barrier()
            ...
            if rank==0:
                ...
                if (np.shape(utilde)[0] < Ny):
                    print("ERROR: Shape mismatch".format(np.shape(utilde)))
                    sys.exit()
                else:
                    outputfile = filename_prefix + "mean_r" + qname + "_%04d.dat"%tid
