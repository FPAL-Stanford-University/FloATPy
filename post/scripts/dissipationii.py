...
if __name__ == '__main__':
    ...
    for tID in tID_list: 
        ...
        tau33 = bmbda*dwdz + lmbda*(dudx + dvdy)
        ...
        tmp = tau11*dudx + tau12*dudy + tau13*dudz;
        upp,dudx,dudy,dudz = None,None,None,None
        if rank==0: print('Computing grad vpp')
        dvdx,dvdy,dvdz = der.gradient(vpp, x_bc=x_bc, y_bc=y_bc, z_bc=z_bc)
        tmp += tau12*dvdx + tau22*dvdy + tau23*dvdz
        vpp,dvdx,dvdy,dvdz = None,None,None,None 
        if rank==0: print('Computing grad wpp')
        dwdx,dwdy,dwdz = der.gradient(wpp, x_bc=x_bc, y_bc=y_bc, z_bc=z_bc)
        tmp += tau13*dwdx + tau23*dwdy + tau33*dwdz
        wpp,dwdx,dwdy,dwdz = None,None,None,None 
       
        tau12,tau13,tau23=None,None,None
        tau11,tau22,tau33=None,None,None

        if (procs>1) and (np.shape(tmp)[1]<Ny): 
            if rank==0: print('Transposing')
            tmp = transpose2y(settings,tmp)
        Dii = np.squeeze(stats.reynolds_average(avg,tmp))
        tmp=None

        if rank==0: 
            outputfile = dir_out+"/dissipationii_%04d.dat"%tID
            np.savetxt(outputfile,Dii,delimiter=' ')
