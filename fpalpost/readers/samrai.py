...
class samraiDataReader:
    ...
    def readSummary(self):
        ...
        if not self.__data_directory_path:
            raise IOError('The data directory path is not set yet!')
        ...
        if numpy.char.strip(self.__basic_info['grid_type']) != 'CARTESIAN':
            raise IOError("Grid type other than 'CARTESIAN' not supported!")
    ...
    def getBasicInfo(self):
        ...
        if not self.__summary_loaded:
            raise IOError('The summary file is not read yet!')
    ...
    def getData(self):
        ...
        if not self.__data_loaded:
            raise IOError('No data is read yet!')

        return self.__x_coords, self.__y_coords, self.__z_coords, self.__data
    ...
    def readDataAtOneLevel(self, \
            ...
            level_num):
        """
        Read data at one particiular level.
        """
        ...
        # Initialize container to store the combined coordinates and data. The elements in the container
        ...
        data_shape = up_level - lo_level + numpy.ones(3, dtype = numpy.int)
        
        if dim == 1:
            data_shape = numpy.delete(data_shape, [1, 2])
            ...
            data_shape = numpy.delete(data_shape, [2])
            ...
            raise IOError('Problem dimension < 1 or > 3 not supported!')
            ...
            for process_idx in range(0, num_file_clusters):
                ...
                for component_idx in range(0, var_num_components):
                    for patch_key in file_cluster_level:
                        ...
                        patch_shape = numpy.delete(patch_shape, [1, 2])
            ...
            for process_idx in range(0, num_file_clusters):
                ...
                for component_idx in range(0, var_num_components):
                    for patch_key in file_cluster_level:
                        ...
                        patch_shape = up_patch - lo_patch + numpy.ones(3, dtype = numpy.int)
                        patch_shape = numpy.delete(patch_shape, [2])
    ...
    def readCombinedDataFromAllLevels(self, \
        ...
        data_shape = up_root_level - lo_root_level + numpy.ones(3, dtype = numpy.int)
        data_shape = numpy.multiply(data_shape, ratios_to_finest_level[0])
        
        if dim == 1:
            data_shape = numpy.delete(data_shape, [1, 2])
            
            self.__x_coords = numpy.linspace(x_lo_root_level[0] + 0.5*dx[-1][0], \
                x_up_root_level[0] - 0.5*dx[-1][0], \
                num = data_shape[0])
            ...
            self.__x_coords = numpy.linspace(x_lo_root_level[0] + 0.5*dx[-1][0], \
                x_up_root_level[0] - 0.5*dx[-1][0], \
                num = data_shape[0])
            self.__y_coords = numpy.linspace(x_lo_root_level[1] + 0.5*dx[-1][1], \
                x_up_root_level[1] - 0.5*dx[-1][1], \
                num = data_shape[1])
            ...
            self.__y_coords = numpy.linspace(x_lo_root_level[1] + 0.5*dx[-1][1], \
                x_up_root_level[1] - 0.5*dx[-1][1], \
                num = data_shape[1])
            self.__z_coords = numpy.linspace(x_lo_root_level[2] + 0.5*dx[-1][2], \
                x_up_root_level[2] - 0.5*dx[-1][2], \
                num = data_shape[2])
                ...
                for level_num in range(num_levels):
                    ...
                    for component_idx in range(0, var_num_components):
                        for patch_key in file_cluster_level:
                            ...
                            x_start_idx = lo_patch[0]*ratios_to_finest_level[level_num][0]
                            x_end_idx = (up_patch[0] + 1)*ratios_to_finest_level[level_num][0]
                ...
                for level_num in range(num_levels):
                    ...
                    for component_idx in range(0, var_num_components):
                        for patch_key in file_cluster_level:
                            ...
                            y_start_idx = lo_patch[1]*ratios_to_finest_level[level_num][1]
                            y_end_idx = (up_patch[1] + 1)*ratios_to_finest_level[level_num][1]
                ...
                for level_num in range(num_levels):
                    ...
                    for component_idx in range(0, var_num_components):
                        for patch_key in file_cluster_level:
                            ...
                            z_start_idx = lo_patch[2]*ratios_to_finest_level[level_num][2]
                            z_end_idx = (up_patch[2] + 1)*ratios_to_finest_level[level_num][2]
    ...
    def readCombinedDataInSubdomainFromAllLevels(self, \
            ...
            periodic_dimension):
        """
        Read data in a sub-domain from all levels, refine the data and combine the data at the highest
        level resolution.
        """
        ...
        if numpy.all(numpy.less_equal(lo_subdomain, up_subdomain)) == False:
            raise IOError('lo_subdomain is greater than up_subdomain!')
        ...
        lo_root_level_refined = numpy.multiply(lo_root_level, ratios_to_finest_level[0])
        up_root_level_refined = numpy.multiply(up_root_level + numpy.ones(3, dtype = numpy.int), ratios_to_finest_level[0]) \
                                - numpy.ones(3, dtype = numpy.int)
        
        # Compute the shape of the domain refined to the highest level.
        
        domain_shape = up_root_level_refined - lo_root_level_refined + numpy.ones(3, dtype = numpy.int)
        
        if dim == 1:
            domain_shape = numpy.delete(domain_shape, [1, 2])
        elif dim == 2:
            domain_shape = numpy.delete(domain_shape, [2])
        elif (dim < 1) or (dim > 3):
            raise IOError('Problem dimension < 1 or > 3 not supported!')
        ...
        if dim == 1:
            lo_root_level_refined = numpy.delete(lo_root_level_refined, [1, 2])
            up_root_level_refined = numpy.delete(up_root_level_refined, [1, 2])
            
            if numpy.all(numpy.greater_equal(numpy.delete(lo_subdomain, [1, 2]), lo_root_level_refined)) == False:
                raise IOError('Input sub-domain not inside the computational domain!')
            if numpy.all(numpy.less_equal(numpy.delete(up_subdomain, [1, 2]), up_root_level_refined)) == False:
                raise IOError('Input sub-domain not inside the computational domain!')
        
        elif dim == 2:
            lo_root_level_refined = numpy.delete(lo_root_level_refined, [2])
            up_root_level_refined = numpy.delete(up_root_level_refined, [2])
            
            if numpy.all(numpy.greater_equal(numpy.delete(lo_subdomain, [2]), lo_root_level_refined)) == False:
                raise IOError('Input sub-domain not inside the computational domain!')
            if numpy.all(numpy.less_equal(numpy.delete(up_subdomain, [2]), up_root_level_refined)) == False:
                raise IOError('Input sub-domain not inside the computational domain!')
        
        elif dim == 3:
            if numpy.all(numpy.greater_equal(lo, lo_root_level_refined)) == False:
                raise IOError('Input sub-domain not inside the computational domain!')
            if numpy.all(numpy.less_equal(up, up_root_level_refined)) == False:
                raise IOError('Input sub-domain not inside the computational domain!')
        ...
        lo_subdomain = lo_subdomain - num_ghosts
        up_subdomain = up_subdomain + num_ghosts
        ...
        for level_num in range(num_levels):
            ...
            for patch_idx in range(num_patches[level_num]):
                ...
                if dim == 1:
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1):
                        if (up_patch_refined[0] + num_ghosts[0] > up_root_level_refined[0]):
                            load_file_clusters[file_cluster_num] = True
                    
                    # Check the right boundary.
                    
                    if (up_subdomain[0] == up_root_level_refined[0] + num_ghosts[0]) and (periodic_dimension[0] == 1):
                    ...
                    if (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1):
                    ...
                    if (up_subdomain[1] == up_root_level_refined[1] + num_ghosts[1]) and (periodic_dimension[1] == 1):
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1):
                        if (up_patch_refined[0] + num_ghosts[0] > up_root_level_refined[0]) and \
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (up_subdomain[1] == up_root_level_refined[1] + num_ghosts[1]) and (periodic_dimension[1] == 1):
                        if (up_patch_refined[0] + num_ghosts[0] > up_root_level_refined[0]) and \
                    ...
                    if (up_subdomain[0] == up_root_level_refined[0] + num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1):
                        if (lo_patch_refined[0] - num_ghosts[0] < lo_root_level_refined[0]) and \
                           (up_patch_refined[1] + num_ghosts[1] > up_root_level_refined[1]):
                            load_file_clusters[file_cluster_num] = True
                    ...
                    if (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1) and \
                    ...
                    if (up_subdomain[1] == up_root_level_refined[1] + num_ghosts[1]) and (periodic_dimension[1] == 1) and \
                       (lo_patch_refined[0] <= up_subdomain[0]) and (up_patch_refined[0] >= lo_subdomain[0]) and \
                       (lo_patch_refined[2] <= up_subdomain[2]) and (up_patch_refined[2] >= lo_subdomain[2]):
                    ...
                    if (lo_subdomain[2] == lo_root_level_refined[2] - num_ghosts[2]) and (periodic_dimension[2] == 1) and \
                       (lo_patch_refined[0] <= up_subdomain[0]) and (up_patch_refined[0] >= lo_subdomain[0]) and \
                       (lo_patch_refined[1] <= up_subdomain[1]) and (up_patch_refined[1] >= lo_subdomain[1]):
                    ...
                    if (up_subdomain[2] == up_root_level_refined[2] + num_ghosts[2]) and (periodic_dimension[2] == 1) and \
                       (lo_patch_refined[0] <= up_subdomain[0]) and (up_patch_refined[0] >= lo_subdomain[0]) and \
                       (lo_patch_refined[1] <= up_subdomain[1]) and (up_patch_refined[1] >= lo_subdomain[1]):
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1) and \
                       (lo_patch_refined[2] <= up_subdomain[2]) and (up_patch_refined[2] >= lo_subdomain[2]):
                            load_file_clusters[file_cluster_num] = True
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (up_subdomain[1] == up_root_level_refined[1] + num_ghosts[1]) and (periodic_dimension[1] == 1) and \
                       (lo_patch_refined[2] <= up_subdomain[2]) and (up_patch_refined[2] >= lo_subdomain[2]):
                            load_file_clusters[file_cluster_num] = True
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (lo_subdomain[2] == lo_root_level_refined[2] - num_ghosts[2]) and (periodic_dimension[2] == 1) and \
                       (lo_patch_refined[1] <= up_subdomain[1]) and (up_patch_refined[1] >= lo_subdomain[1]):
                            load_file_clusters[file_cluster_num] = True
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (up_subdomain[2] == up_root_level_refined[2] + num_ghosts[2]) and (periodic_dimension[2] == 1) and \
                       (lo_patch_refined[1] <= up_subdomain[1]) and (up_patch_refined[1] >= lo_subdomain[1]):
                            load_file_clusters[file_cluster_num] = True
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1) and \
                       (lo_subdomain[2] == lo_root_level_refined[2] - num_ghosts[2]) and (periodic_dimension[2] == 1):
                        if (up_patch_refined[0] + num_ghosts[0] > up_root_level_refined[0]) and \
                    ...
                    if (lo_subdomain[0] == lo_root_level_refined[0] - num_ghosts[0]) and (periodic_dimension[0] == 1) and \
                       (lo_subdomain[1] == lo_root_level_refined[1] - num_ghosts[1]) and (periodic_dimension[1] == 1) and \
                       (up_subdomain[2] == up_root_level_refined[2] + num_ghosts[2]) and (periodic_dimension[2] == 1):
                        if (up_patch_refined[0] + num_ghosts[0] > up_root_level_refined[0]) and \
                           (up_patch_refined[1] + num_ghosts[1] > up_root_level_refined[1]) and \
        ...
        #Initialize container to store the combined coordinates and data. The elements in the container are initialized as NAN values.
        
        data_shape = up_root_level - lo_root_level + numpy.ones(3, dtype = numpy.int)
        data_shape = numpy.multiply(data_shape, ratios_to_finest_level[0]) + 2*num_ghosts
        ...
        data_shape = up_subdomain - lo_subdomain + numpy.ones(3, dtype = numpy.int)
        
        if dim == 1:
            data_shape = numpy.delete(data_shape, [1, 2])
            lo_subdomain = numpy.delete(lo_subdomain, [1, 2])
            up_subdomain = numpy.delete(up_subdomain, [1, 2])
        elif dim == 2:
            data_shape = numpy.delete(data_shape, [2])
            lo_subdomain = numpy.delete(lo_subdomain, [2])
            up_subdomain = numpy.delete(up_subdomain, [2])
        elif dim < 1 or dim > 3:
            raise IOError('Problem dimension < 1 or > 3 not supported!')
